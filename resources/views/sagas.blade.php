<x-layout>
    ...
    @if ($sagas->isNotEmpty())
        ...
        <article class="py-8 max-w-screen-sm mx-auto border-b border-gray-200">
            <a href="/sagas/{{ $saga['slug'] }}" class="flex items-center">
                <h3 class="text-2xl font-bold mr-2 tracking-tight text-primary-500 w-[60%]">
                    {{ $saga['title'] }}
                </h3>
            ...
            <div class="text-base text-gray-300">
                <a href="/authors-saga/{{ $saga->author->username }}" class="text-sm">{{ $saga->author->name }}</a> | <a href="#" class="text-sm"><span class="text-yellow-300"> {{ $saga->sagaCategory->name }} </span></a> | <a href="#" class="text-sm">{{ $saga->created_at->diffForHumans() }}</a>
            </div>
    ...
    @else
        <article class="py-8 max-w-screen-sm mx-auto">
            <div class="text-center text-base text-gray-500">Belum ada kisah &#x1F61E;</div>
        </article>
    @endif
</x-layout>